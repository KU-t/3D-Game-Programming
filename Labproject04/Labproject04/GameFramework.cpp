...
void GameFramework::CreateSwapChain() {
	...
	//p13 RGBA 8,8,8,8비트 [0~1]로 표현
	dxgiSwapChainDesc.BufferDesc.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
	//1s에 60hz지정
	dxgiSwapChainDesc.BufferDesc.RefreshRate.Numerator = 60; // 분모
	dxgiSwapChainDesc.BufferDesc.RefreshRate.Denominator = 1; // 분자
	//p12 ☆☆☆후면버퍼에대한 표면 사용방식, CPU접근방법을 렌터타겟으로 이용
	dxgiSwapChainDesc.BufferUsage = DXGI_USAGE_RENDER_TARGET_OUTPUT;
	//p12 스와핑처링방식을 버퍼내용물 제거방식으로
	dxgiSwapChainDesc.SwapEffect = DXGI_SWAP_EFFECT_FLIP_DISCARD;
	//출력 윈도우
	dxgiSwapChainDesc.OutputWindow = m_hWnd;
	//다중샘플링 품질지정 
	//m_bMsaa4xEnable == 4배 가능 여부
...
void GameFramework::CreateDirect3DDevice() {
	...
	//DXGI 팩토리를 생성한다.(포인터 변수 선언, 포인터 변수의 주소를 넘김)
	...
	//컴퓨터에 장착된 그래픽카드 개수 확인 -> Direct3D를 디바이스를 생성하기 위한 그래픽카드 선택 (원하는 능력을 가진)
	//EnumAdapters1 -> 그래픽 어댑터 열거하기 (인덱스를 벗어나면 DXGI_ERROR_NOT_FOUND반환
	for (UINT i = 0; DXGI_ERROR_NOT_FOUND != m_pdxgiFactory->EnumAdapters1(i, &pd3dAdapter); i++) {
		...
		if (dxgiAdapterDesc.Flags & DXGI_ADAPTER_FLAG_SOFTWARE)
			continue;
		// 어댑터(pd3dAdapter : 반복문을 돌면서 어댑터의 능력을 가져옴)가 특성 레벨12.0.(D3D_FEATURE_LEVEL_12_0)을
		// 만드는 능력(SUCCEEDED(D3D12CreateDevice)이 있다면 내 어댑터(m_pd3dDevice)주소를 넘기고 그만찾아
		if (SUCCEEDED(D3D12CreateDevice(pd3dAdapter, D3D_FEATURE_LEVEL_12_0, _uuidof(ID3D12Device), (void **)&m_pd3dDevice)))
			break;
	} 

	//특성 레벨 12.0을 지원하는 하드웨어 디바이스를 생성할 수 없으면 WARP 디바이스를 생성한다.
	//능력이 되는 어댑터가 없다면 WARP 디바이스로 생성해
	// ################### 추가해
	if (!pd3dAdapter) {
...
void GameFramework::FrameAdvance() {
	...
	렌더 타겟 의 상태는 프리젠트 상태(D3D12_RESOURCE_STATE_PRESENT)로 바뀔 것이다.*/
