...
inline void GameFramework::CreateSwapChain() {
	...
	dxgiSwapChainDesc.BufferDesc.Format = DXGI_FORMAT_R8G8B8A8_UNORM; // 후면버퍼 픽셀 형식
	dxgiSwapChainDesc.SampleDesc.Count = (m_bMsaa4xEnable) ? 4 : 1;
	dxgiSwapChainDesc.SampleDesc.Quality = (m_bMsaa4xEnable) ? (m_nMsaa4xQualityLevels - 1) : 0;
	dxgiSwapChainDesc.BufferUsage = DXGI_USAGE_RENDER_TARGET_OUTPUT; //후면버퍼에 대한 표면 사용 방식과 CPU의 접근 방법
	dxgiSwapChainDesc.BufferCount = m_nSwapChainBuffers; //후면버퍼의 수를 지정
	dxgiSwapChainDesc.SwapEffect = DXGI_SWAP_EFFECT_FLIP_DISCARD; //스와핑을 처리하는 선택사항을 지정
	dxgiSwapChainDesc.Flags = DXGI_SWAP_CHAIN_FLAG_ALLOW_MODE_SWITCH; //스왑체인의 동작에 대한 선택사항을 지정(보통0)
	//[따라하기5: 전체화면 모드에서 바탕화면의 해상도를 스왑체인(후면버퍼)의 크기에 맞게 변경한다.]
	dxgiSwapChainDesc.Windowed = TRUE;
	dxgiSwapChainDesc.BufferDesc.RefreshRate.Numerator = 60; 
	dxgiSwapChainDesc.BufferDesc.RefreshRate.Denominator = 1;
	dxgiSwapChainDesc.OutputWindow = m_hWnd;
	//dxgiSwapChainDesc.BufferDesc.Scaling = DXGI_SCALING_NONE; // 모니터 해상도에 맞게 확대하는 방법
...
void GameFramework::CreateDirect3DDevice() {
	...
	for (UINT i = 0; DXGI_ERROR_NOT_FOUND != m_pdxgiFactory->EnumAdapters1(i, &pd3dAdapter); i++) {
		...
		if (SUCCEEDED(D3D12CreateDevice(pd3dAdapter, D3D_FEATURE_LEVEL_12_0, _uuidof(ID3D12Device),
...
void GameFramework::CreateCommandQueueAndList(){
	...
	d3dCommandQueueDesc.Type = D3D12_COMMAND_LIST_TYPE_DIRECT; //GPU가 직접 실행할수 있는 명령 버퍼(리스트)

	HRESULT hResult = m_pd3dDevice->CreateCommandQueue
	(&d3dCommandQueueDesc, _uuidof(ID3D12CommandQueue), (void **)&m_pd3dCommandQueue);
...
void GameFramework::CreateDepthStencilViews(){
	...
	m_pd3dDevice->CreateCommittedResource(&d3dHeapProperties,
...
void GameFramework::BuildObjects(){
	...
	m_pScene = new Scene(); m_pScene->BuildObjects(m_pd3dDevice, m_pd3dCommandList);
