...
CMesh * CGameFramework::BulidCoordinate(){
	CMesh *pCoordinate = new CMesh(2);

	CPolygon *plane = new CPolygon(2);
	plane->SetVertex(0, CVertex(0, 0, 0, RGB(255, 0, 0)));
	plane->SetVertex(1, CVertex(1, 0, 0, RGB(0, 0, 0)));
	pCoordinate->SetPolygon(0, plane);

	CPolygon *triangle = new CPolygon(3);
	triangle->SetVertex(0, CVertex(1, 0.2, 0, RGB(0, 0, 255)));
	triangle->SetVertex(1, CVertex(1.5, 0, 0, RGB(0, 0, 255)));
	triangle->SetVertex(2, CVertex(1.0, -0.2, 0, RGB(0, 0, 255)));
	pCoordinate->SetPolygon(1, triangle);

	return (pCoordinate);
}

CMesh * CGameFramework::BuildRollerCoster(void) {
	CMesh *pRollerCoster = new CMesh(10);
...
void CGameFramework::BuildObjects(){
	...
	CMesh *pCoordinate = BulidCoordinate();
	int Coordinates = 3;

	CMesh *pRollerCoster = BuildCubeMesh();
	int RollerCosters = 1;

	m_nObjects = Cubes + Coordinates + RollerCosters;
	m_pObjects = new CGameObject[m_nObjects];

    m_pObjects[0].SetMesh(pMesh);
    m_pObjects[0].SetPosition(-3.5, +2, +14.0f);
    m_pObjects[1].SetMesh(pMesh);
    m_pObjects[1].SetPosition(+3.5f, -2.0f, +14.0f);

	m_pObjects[2].SetMesh(pCoordinate);
	m_pObjects[2].SetPosition(-0, +0, +14.0f);

	m_pObjects[3].SetMesh(pCoordinate);
	m_pObjects[3].SetRotation(-0, +0, +90.0f);
	m_pObjects[3].SetPosition(-0, +0, +14.0f);

	m_pObjects[4].SetMesh(pCoordinate);
	m_pObjects[4].SetRotation(-0, +90.f, 0.f);
	m_pObjects[4].SetPosition(-0, +0, +14.0f);

	m_pObjects[5].SetMesh(pRollerCoster);
	m_pObjects[5].SetRotation(-0, 0.f, 0.f);
	m_pObjects[5].SetPosition(-0, +0, +10.0f);
...
void CGameFramework::ProcessInput() {
    ...
    if (GetKeyState(VK_LEFT) & 0xFF00) m_pCamera->m_fxPosition -= 25.0f * 0.0005f;
    if (GetKeyState(VK_RIGHT) & 0xFF00) m_pCamera->m_fxPosition += 25.0f * 0.0005f;
    if (GetKeyState(VK_UP) & 0xFF00) m_pCamera->m_fyPosition += 25.0f * 0.0005f;
    if (GetKeyState(VK_DOWN) & 0xFF00) m_pCamera->m_fyPosition -= 25.0f * 0.0005f;
}
